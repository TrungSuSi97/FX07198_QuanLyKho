...
namespace TPH.LIS.App.CauHinh.NhanVien
{
    public partial class frmNhanVien_ChanDoan : FrmTemplate
    {
        ...
        {
            repositoryItemGridLookUpEditDonVi.DataSource = iConfig.GetLocation(string.Empty, string.Empty, string.Empty);
        }
        private void Load_Phong()
        {
            repositoryItemGridLookUpEditPhong.DataSource = iConfig.Data_dm_phong(string.Empty, string.Empty, string.Empty);
           ...
           if(gvNhanVien.FocusedRowHandle > -1 )
            {
                ...
                {
                    if(CustomMessageBox.MSG_Question_YesNo_GetYes("Xóa nhân viên/Bác Sĩ đang chọn?"))
                    {
       ...
       private void CapNhatCTV()
        ...
        private void gvNhanVien_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if(e.RowHandle >-1)
            {
                ...
                obj.Chucvu = gvNhanVien.GetRowCellValue(e.RowHandle, colChucVu).ToString();
                obj.Chungchihanhnghe = gvNhanVien.GetRowCellValue(e.RowHandle, colchungchihanhnghe).ToString();
                ...
                obj.Nhomnhanvien = gvNhanVien.GetRowCellValue(e.RowHandle, colMaNhom).ToString();
                obj.Thongke = bool.Parse(gvNhanVien.GetRowCellValue(e.RowHandle, colThongKe).ToString());
                obj.Makhoaphong = gvNhanVien.GetRowCellValue(e.RowHandle, colMaKhoaPhong).ToString();
                obj.Maphong = gvNhanVien.GetRowCellValue(e.RowHandle, colMaPhong).ToString();
                obj.Nguoisua = CommonAppVarsAndFunctions.UserID;
        ...
        private void repositoryItemGridLookUpEditPhong_Popup(object sender, EventArgs e)
        {
            ...
            if (gvNhanVien.FocusedRowHandle > -1)
            {
                var makhoa = gvNhanVien.GetRowCellValue(gvNhanVien.FocusedRowHandle, colMaKhoaPhong).ToString();
