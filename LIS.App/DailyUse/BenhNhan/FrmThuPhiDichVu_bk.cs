...
{
    public partial class FrmThuPhiDichVu_bk : FrmTemplate
    {
        ...
        private void InBienLai(string idBienLai, string _MaTiepNhan, List<CashierItemSelected> selectedList, bool batThuTien, bool xemTruoc)
        {
            if (string.IsNullOrEmpty(idBienLai)) return;
            var inBienLai = In_BienLai(xemTruoc, LabServices_Helper.Get_PrinterSelected(listPrinterCharge), idBienLai,
                EnumThaoTacThuPhi.ThuTien);
            if (inBienLai)
            {
        ...
        public bool In_BienLai(bool isReView, string printerName, string idBienLai, EnumThaoTacThuPhi loai)
        {

            var dataPrint = _isCashier.Data_DanhSachChiDinh_BL(idBienLai, loai, EnumChon.TatCa);
        ...
        private void FrmThuPhiDichVu_Load(object sender, EventArgs e)
        {
            ...
            user.Get_UserSign(cboUsersign, ServiceType.ThuNgan, " and d.MaNguoiDung not in ('admin')");
            user.Get_UserSign(cboThuNgan2, ServiceType.ThuNgan, " and d.MaNguoiDung not in ('admin')");
            user.Get_UserSign(cboThuNganHoantien, ServiceType.ThuNgan, " and d.MaNguoiDung not in ('admin')");
        ...
        private void btnInBienLai_BienLai_Click(object sender, EventArgs e)
        {
            if (gvBienLai.FocusedRowHandle > -1)
            {
                ...
                if (list.Count > 0)
                {
                    InBienLai(idBienlai, idMatiepNhan, list, chkBienLai_BatThuTien.Checked, !chkBienLai_inTrucTiep.Checked);
                }
                Load_ChitietBienLai();
            }
        }
