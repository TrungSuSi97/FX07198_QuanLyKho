...
{
    public partial class FrmInput_List : FrmTemplate
    {
        public FrmInput_List()
        {
            InitializeComponent();
        }
        ...
        private void Search_input_Thuoc()
        {
            string _MaNhomVatTu = (cboNhomVatTu.SelectedIndex != -1 ? cboNhomVatTu.SelectedValue.ToString() : "");
            string _MaNhaCungCap = (cboNhaCungCap.SelectedIndex == -1 ? "" : cboNhaCungCap.SelectedValue.ToString().Trim());
            string _VatTu = (cboVatTu.SelectedIndex == -1 ? "" : cboVatTu.SelectedValue.ToString().Trim());
            DataTable dt = phieuNhapDAL.Data_Search_InputList(dtpFromDate.Value, dtpToDate.Value, txtTimSoPhieu.Text, txtNguoiLap.Text, _MaNhaCungCap, _MaNhomVatTu, _VatTu);
        ...
        private void Load_Detail()
        {
            ...
            if (IndexLoaiVatTu.Trim().ToUpper().Equals("THUOC"))
            {
                ControlExtension.BindDataToGrid(chitietDAL.Data_VT_NhapKho_ChietTiet_Thuoc("", _MaSoPhieu, ""), ref dtgDetail, ref bvDetail);
            }
        }
