...
{
    public partial class frmThongKeTongHopSA : FrmTemplate
    {
        public frmThongKeTongHopSA()
        {
            InitializeComponent();
        }
        ...
        private string Get_Condit()
        {
            ...
            if (cboCLS_ChiDinh.SelectedIndex != -1)
            {
                ...
                _condit += " and K.MaSoMau ='" + cboCLS_ChiDinh.SelectedValue.ToString().Trim() + "'\n";
            }

            //Nếu chọn nhóm
            if (cboNhomCLS.SelectedIndex != -1)
            {
                _condit += " and K.MaNhomSieuAm = '" + cboNhomCLS.SelectedValue.ToString().Trim() + "'\n";
            }
        ...
        private void ThongKeDoiTuongDichVu()
        {
            ...
            _strSQL += " from BenhNhan_TiepNhan T inner join BenhNhan_CLS_SieuAm Tx on t.MaTiepNhan = tx.MaTiepNhan\n inner join KetQua_CLS_SieuAm K on T.MaTiepNhan = K.MaTiepNhan \n";
        ...
        {
            string _strSQL = "";
            _strSQL += " select isnull(N.TenNhanVien, N'Chưa in') as TenNhanVien , isnull(Tx.UserInSieuAm,'None') as UserInSieuAm ,K.MaNhomSieuAm, C.TenNhomSieuAm,  Sum(K.GiaRieng) as TongTien, Count(distinct K.MaTiepNhan) as SoLuong, cast(0 as bit) as IsCate\n";
            ...
            //Ghép với điều kiện
            _strSQL += Get_Condit();
            _strSQL += " group by N.TenNhanVien, Tx.UserInSieuAm,K.MaNhomSieuAm, C.TenNhomSieuAm \n";
            _strSQL += " order by K.MaNhomSieuAm,N.TenNhanVien asc";
            ...
            LabServices_Helper.TachNhom_Datatable(dtTemp, ref dtResult, "TenNhomSieuAm", "MaNhomSieuAm", "TenNhanVien", "TongTien", "SoLuong");
            ControlExtension.BindDataToGrid(dtResult, ref dtgNguoiThucHien, ref bvNguoiThuchien);
        }

        //Lấy thống kê tổng số xét nghiệm

        private void TK_SieuAmThucHien()
        {
            string _strSQL = "";
            _strSQL += " Select K.MaSoMau, K.TenMauSieuAm, K.MaNhomSieuAm, C.TenNhomSieuAm, Sum(GiaRieng) as TongTien, Count(MaSoMau) as SoLuong, cast(0 as bit) as IsCate \n";
            _strSQL += " from BenhNhan_TiepNhan T inner join BenhNhan_CLS_SieuAm Tx on t.MaTiepNhan = tx.MaTiepNhan\n inner join KetQua_CLS_SieuAm K on T.MaTiepNhan = K.MaTiepNhan \n";
            _strSQL += " left join {{TPH_Standard}}_Dictionary.dbo.DM_NhomCLS_SieuAm C on K.MaNhomSieuAm = C.MaNhomSieuAm \n";
            //Ghép với điều kiện
            _strSQL += Get_Condit();
            _strSQL += " group by K.MaSoMau, K.TenMauSieuAm, K.MaNhomSieuAm, C.TenNhomSieuAm \n";
            _strSQL += " order by  c.TenNhomSieuAm, k.MaNhomSieuAm,K.TenMauSieuAm,  K.MaSoMau asc";
            DataTable dtTemp = new DataTable();
            dtTemp = DataProvider.ExecuteDataset(CommandType.Text, _strSQL).Tables[0];
            DataTable dtResult = new DataTable();
            dtResult = dtTemp.Clone();

            LabServices_Helper.TachNhom_Datatable(dtTemp, ref dtResult, "TenNhomSieuAm", "MaNhomSieuAm", "TenMauSieuAm", "TongTien", "SoLuong");
            ControlExtension.BindDataToGrid(dtResult, ref dtgTongHopSieuAm, ref bvTongHopSieuAm);
        }

        private void frmThongKeTongHopSA_Load(object sender, EventArgs e)
        {
           ...
           // data.Get_NhanVien(cboNhanVienThucHien);
            ...
            sysConfig.Get_NhomCLS_SieuAm(cboNhomCLS, "");
            Get_ChiDinhCLS();
        ...
        {
            sysConfig.Get_DMSieuAm(cboCLS_ChiDinh, (cboNhomCLS.SelectedIndex == -1 ? "" : " and MaNhomSieuAm = '" + cboNhomCLS.SelectedValue.ToString().Trim() + "'"), "");
        }
        ...
        private void dtgTongHopSieuAm_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            LabServices_Helper.DanhDauNhom_STT(dtgTongHopSieuAm);
        }

        private void btnXuatTKSA_Click(object sender, EventArgs e)
        {
            if (dtgTongHopSieuAm.RowCount > 0)
            {
               Excel.ExportToExcel.Export(dtgTongHopSieuAm);
            }
        }
        ...
        {
            string _strSQL = "";
            _strSQL += " select T.TenBN,T.Tuoi,T.DiaChi, Th.TenNhanVien as BSThucHien,t.ThoiGianNhap as NgayTiepNhan, Case when T.GioiTinh = 'M' then N'Nam' when T.GioiTinh = 'F' then N'Nữ' else N'?' end as GioiTinh, D.TenDoiTuongDichVu,N.TenNhanVien as BacSiCD, T.DoiTuongDichVu , Sum(K.GiaRieng) as TongTien, Count(distinct K.MaSoMau) as SoLuong,cast (0 as bit) as isCate\n";
            _strSQL += " from BenhNhan_TiepNhan T inner join KetQua_CLS_SieuAm K on T.MaTiepNhan = K.MaTiepNhan inner join BenhNhan_CLS_SieuAm tx on tx.MaTiepNhan = t.MaTiepNhan \n";
            _strSQL += " left join {{TPH_Standard}}_Dictionary.dbo.DM_DoiTuongDichVu D on d.MaDoiTuongDichVu = T.DoiTuongDichVu \n";
        ...
        private void ThongKeTH_Chitiet_BenhNhan()
        {
           ...
           // _strSQL += " and Tx.DaInKQSieuAm = 1";
            ...
            DataTable dtTemp = new DataTable();
            dtTemp = DataProvider.ExecuteDataset(CommandType.Text, _strSQL).Tables[0];
            LabServices_Helper.TachNhom_Datatable(dtTemp, ref dtResult, "NguoiThucHien", "UserInSieuAm", "TenBN", "TongTien", "SoLuong");
        ...
        {
            string _strSQL = "";
            _strSQL += " Select K.NguoiKy as UserInSieuAm , Th.TenNhanVien as NguoiThucHien,K.MaSoMau, K.TenMauSieuAm, K.MaNhomSieuAm, C.TenNhomSieuAm, Sum(GiaRieng) as TongTien, Count(MaSoMau) as SoLuong, cast(0 as bit) as IsCate \n";
            _strSQL += " from BenhNhan_TiepNhan T inner join KetQua_CLS_SieuAm K on T.MaTiepNhan = K.MaTiepNhan inner join BenhNhan_CLS_SieuAm tx on tx.MaTiepNhan = t.MaTiepNhan \n";
            _strSQL += " left join {{TPH_Standard}}_Dictionary.dbo.DM_NhomCLS_SieuAm C on K.MaNhomSieuAm = C.MaNhomSieuAm \n";
            _strSQL += " left join (select nd.MaNguoiDung,nv.TenNhanVien from {{TPH_Standard}}_System.dbo.QL_NguoiDung nd inner join {{TPH_Standard}}_Dictionary.dbo.QL_NHANVIEN nv on nd.MaNhanVien = nv.MaNhanVien) as th on th.MaNguoiDung =  K.NguoiKy ";
            //Ghép với điều kiện
            ...
            _strSQL += " group by K.NguoiKy, Th.TenNhanVien, K.MaSoMau, K.TenMauSieuAm, K.MaNhomSieuAm, C.TenNhomSieuAm \n";
            _strSQL += " order by K.NguoiKy, K.TenMauSieuAm,  K.MaSoMau asc";
            DataTable dtTemp = new DataTable();
            dtTemp = DataProvider.ExecuteDataset(CommandType.Text, _strSQL).Tables[0];
            DataTable dtResult = new DataTable();
            dtResult = dtTemp.Clone();

            LabServices_Helper.TachNhom_Datatable(dtTemp, ref dtResult, "NguoiThucHien", "UserInSieuAm", "TenMauSieuAm", "TongTien", "SoLuong");
