...
{
    public partial class ucSearchLookupEditor_District : UserControl
    {
        private readonly ISystemConfigService sysConfig = new SystemConfigService();
        public ucSearchLookupEditor_District()
        {
            InitializeComponent();
        ...
        [Category("Custom")]
        public string GetObjectIdByName(string objectName, int provinceId)
        {
            if (lueTinh.Properties.DataSource == null)
                return string.Empty;
            else
            {
                var dataFound = WorkingServices.SearchResult_OnDatatable((DataTable)lueTinh.Properties.DataSource, string.Format("QuanHuyen = '{0}' and MaTinh = {1}", objectName.Trim(), provinceId));
