...
{
    public partial class ucSearchLookupEditor_Profile : UserControl
    {
        private readonly ISystemConfigService sysConfig = new SystemConfigService();
        public ucSearchLookupEditor_Profile()
        {
            InitializeComponent();
        ...
        public object SelectedValue
        {
            get
            {
                return lueData.EditValue;
            }

            set
            {
                lueData.EditValue = value;
            }
        }
        [Category("Custom")]
        public string ValueMember
        {
            get
            {
                return lueData.Properties.ValueMember;
            }

            set
            {
                lueData.Properties.ValueMember = value;
            }
        }
        [Category("Custom")]
        public string DisplayMember
        {
            get
            {
                return lueData.Properties.DisplayMember;
            }

            set
            {
                lueData.Properties.DisplayMember = value;
            }
        }
        [Category("Custom")]
        public object DataSource
        {
            get
            {
                return lueData.Properties.DataSource;
            }
        ...
        {
            if (lueData.Properties.DataSource == null)
                return string.Empty;
            else
            {
                var dataFound = WorkingServices.SearchResult_OnDatatable((DataTable)lueData.Properties.DataSource, string.Format("TenDoiTuongDichVu = '{0}'", objectName.Trim()));
        ...
        private void lueDoiTuong_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if (txtFind != null && lueData.IsPopupOpen)
            {
                if (!txtFind.Focused)
                {
                    txtFind.Focus();
        ...
        {
            if (gvData.RowCount == 1 && !string.IsNullOrEmpty(txtFind.Text))
            {
        ...
        {
            if (!lueData.IsPopupOpen)
            {
                lueData.ShowPopup();
            }
        }
        public void Load_DataLSS()
        {
            var data = sysConfig.GetData_Profile_SLSS();
            data = WorkingServices.ConvertColumnNameToLower_Upper(data, true);
            lueData.Properties.DataSource = data;
            lueData.Properties.ValueMember = "profileid";
            lueData.Properties.DisplayMember = "profilename";
            lueData.Properties.AutoHeight = false;
            lueData.Properties.BestFitMode = DevExpress.XtraEditors.Controls.BestFitMode.BestFitResizePopup;
        }
    }
}
