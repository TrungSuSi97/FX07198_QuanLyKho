...
{
    public partial class ucSearchLookupEditor_TinhTrangMau : UserControl
    {
        private readonly ISystemConfigService sysConfig = new SystemConfigService();
        public ucSearchLookupEditor_TinhTrangMau()
        {
            InitializeComponent();
        ...
        public void Load_CauHinh(int loaiXetNghiem)
        {
            gcLueId.FieldName = ControlExtension.PropertyNameToLowerCase<DM_TINHTRANGMAU>(x => x.Matinhtrang);
            gcLueId.Caption = ControlExtension.GetDescription<DM_TINHTRANGMAU>(x => x.Matinhtrang);
            gcLueName.FieldName = ControlExtension.PropertyNameToLowerCase<DM_TINHTRANGMAU>(x => x.Tinhtrangmau);
            gcLueName.Caption = ControlExtension.GetDescription<DM_TINHTRANGMAU>(x => x.Tinhtrangmau);
            var data = sysConfig.Data_DanhMucTinhTrangMau(loaiXetNghiem);
            if (data != null)
                data = WorkingServices.SearchResult_OnDatatable(data, string.Format("{0} = {1}", ControlExtension.PropertyNameToLowerCase<DM_TINHTRANGMAU>(x => x.Loaixetnghiem), loaiXetNghiem));
            lueDanhSach.Properties.DataSource = WorkingServices.ConvertColumnNameToLower_Upper(data, true);
            lueDanhSach.Properties.ValueMember = ControlExtension.PropertyNameToLowerCase<DM_TINHTRANGMAU>(x => x.Matinhtrang);
            lueDanhSach.Properties.DisplayMember = ControlExtension.PropertyNameToLowerCase<DM_TINHTRANGMAU>(x => x.Tinhtrangmau);
