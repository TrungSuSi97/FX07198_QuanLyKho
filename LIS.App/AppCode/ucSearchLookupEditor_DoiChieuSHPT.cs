...
{
    public partial class ucSearchLookupEditor_DoiChieuSHPT : UserControl
    {
        private readonly ISystemConfigService sysConfig = new SystemConfigService();
        public ucSearchLookupEditor_DoiChieuSHPT()
        {
            InitializeComponent();
        ...
        [Category("Custom")]
        public string GetSeletetedPosition()
        {
            if (lueBSChiDinh.EditValue == null)
                return string.Empty;
            else
        ...
        public void Load_NguoiDoiCHieuSHPT()
        {
            if (EditValueChanged != null)
                lueBSChiDinh.EditValueChanged -= EditValueChanged;

            var data = userManagement.GetUsersKyTenCoPhanQuyen("", Utilities.ConvertStrinArryToInClauseSQL(CommonAppVarsAndFunctions.BoPhanClsXetNghiem, true), false, true);
