...
namespace OutlookStyleControls
{
    ...
    public class OutlookgGridDefaultGroup : IOutlookGridGroup
    {
        ...
        {
            get { return (string.IsNullOrEmpty(itmsText) ? "item" : itmsText); }
            set { itmsText = value; }
        }
        ...
        public virtual string Text
        {
            get
            {
                itmsText = itmsText ?? "item";
                if (column == null)
                {
                    if (itmsText.Equals("item", StringComparison.OrdinalIgnoreCase))
                        return string.Format("Unbound group: {0} ({1})", Value.ToString(),
                            itemCount == 1 ? "1 item" : itemCount.ToString() + " items");
                    else
                    {
                        return string.Format("Unbound group: {0} ({1})", Value.ToString(),
                            itemCount.ToString() + " " + itmsText);
                    }
                }
                else if (itmsText.Equals("item", StringComparison.OrdinalIgnoreCase))
                    return string.Format("{0}: {1} ({2})", column.HeaderText, Value.ToString(),
                        itemCount == 1 ? "1 item" : itemCount.ToString() + " items");
                else
                {
                    return string.Format("{0}: {1} ({2})", column.HeaderText, Value.ToString(),
