...
{
    public partial class ucSearchLookupEditor_DanhSachKhayLuuMau : UserControl
    {
        private readonly ISystemConfigService sysConfig = new SystemConfigService();
        public ucSearchLookupEditor_DanhSachKhayLuuMau()
        {
            InitializeComponent();
        ...
        public int GetNgang()
        {
            ...
            {
                if (lueDanhSach.EditValue != null)
                {
                    var dataFound = WorkingServices.SearchResult_OnDatatable((DataTable)lueDanhSach.Properties.DataSource, string.Format("{0} = '{1}'", lueDanhSach.Properties.ValueMember, lueDanhSach.EditValue.ToString()));
                    if (dataFound.Rows.Count > 0)
                    {
                        return int.Parse(dataFound.Rows[0][ControlExtension.PropertyNameToLowerCase<DM_KHAYLUUMAU>(x => x.Kichthuocngang)].ToString());
                    }
                }
                return 0;
            }
        ...
        public int GetDoc()
        {
            ...
            else
            {
                if (lueDanhSach.EditValue != null)
                {
                    ...
                    if (dataFound.Rows.Count > 0)
                    {
                        return int.Parse(dataFound.Rows[0][ControlExtension.PropertyNameToLowerCase<DM_KHAYLUUMAU>(x => x.Kichthuocdoc)].ToString());
                    }
                }
                return 0;
            }
        ...
        public void Load_CauHinh(string maTuLuu)
        {
            gcLueId.FieldName = ControlExtension.PropertyNameToLowerCase<DM_KHAYLUUMAU>(x => x.Makhayluumau);
            gcLueId.Caption = ControlExtension.GetDescription<DM_KHAYLUUMAU>(x => x.Makhayluumau);
            gcLueName.FieldName = ControlExtension.PropertyNameToLowerCase<DM_KHAYLUUMAU>(x => x.Tenkhailuumau);
            gcLueName.Caption = ControlExtension.GetDescription<DM_KHAYLUUMAU>(x => x.Tenkhailuumau);
            gcLueGroup.FieldName = ControlExtension.PropertyNameToLowerCase<DM_KHAYLUUMAU>(x => x.Matuluu);
            gcLueGroup.Caption = ControlExtension.GetDescription<DM_KHAYLUUMAU>(x => x.Matuluu);
            var data = sysConfig.Data_dm_khayluumau(maTuLuu, String.Empty);
            lueDanhSach.Properties.DataSource = WorkingServices.ConvertColumnNameToLower_Upper(data, true);
            lueDanhSach.Properties.ValueMember = ControlExtension.PropertyNameToLowerCase<DM_KHAYLUUMAU>(x => x.Makhayluumau);
            lueDanhSach.Properties.DisplayMember = ControlExtension.PropertyNameToLowerCase<DM_KHAYLUUMAU>(x => x.Tenkhailuumau);
