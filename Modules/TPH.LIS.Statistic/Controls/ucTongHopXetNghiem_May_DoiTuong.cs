...
{
    public partial class ucTongHopXetNghiem_May_DoiTuong : UserControl
    {
        public ucTongHopXetNghiem_May_DoiTuong()
        {
            InitializeComponent();
        }
           ...
           lblNgayBaoCao.Text = string.Format(LanguageExtension.GetResourceValueFromValue("Từ ngày {0} đến ngày {1}",
            ...
            if (data != null)
            {
                ...
                foreach (DataRow item in dataDistincDoiTuong.Rows)
                {
                    ...
                    bandUngroupped.Caption = colName;
                    ...
                    var testNum = LanguageExtension.GetResourceValueFromValue("Số test",
                        LanguageExtension.AppLanguage);
                    InsertNewColToGrid(bgvXetNghiem, colSlName, count, colSoLuong.Width, null, null, FormatType.Numeric, "{0:N0}", true);
                    bgvXetNghiem.Columns[bgvXetNghiem.Columns.Count - 1].OwnerBand = bandUngroupped;
                    InsertNewColToGrid(bgvXetNghiem, colHSName, testNum, colSoLuongHeSo.Width, null, null, FormatType.Numeric, "{0:N0}", true);
                    bgvXetNghiem.Columns[bgvXetNghiem.Columns.Count - 1].OwnerBand = bandUngroupped;
                    ...
                    foreach (DataRow drXN in dataResult.Rows)
                    {
                        ...
                        var dataSum = WorkingServices.SearchResult_OnDatatable_NoneSort(data, string.Format("MaXN = '{0}' and DoiTuongDichVu = '{1}'", maXN, maDoiTuong));
                        if (dataSum.Rows.Count > 0)
                        {
                            ...
                            drXN[colTong] = (double.Parse(string.IsNullOrEmpty(drXN[colTong].ToString()) ? "0" : drXN[colTong].ToString())) + tongSo;
                            drXN[colHeSo] = (double.Parse(string.IsNullOrEmpty(drXN[colHeSo].ToString()) ? "0" : drXN[colHeSo].ToString())) + heSo;
                        }
                    }
                }
        ...
        {
            Excel.ExportToExcel.Export(gcXetNghiem,
                string.Format("SoLuongXetnghiemTheoMay_{0}", DateTime.Now.ToString("ddMMyyyyHHmmss")), false, -1, -1,
                new[]
                    {lblTitle.Text, lblNgayBaoCao.Text, lblMayXetNghiem.Text}, new int[] {16, 13, 13},
                new[] {"BỘ QUỐC PHÒNG", "BỆNH VIỆN TWQĐ 108", lblKhoa.Text});
        }

        private void InsertNewColToGrid(GridView gridView, string fieldName, string title,int width = 70, int? minWidth = null, int? maxWidth = null, FormatType? formatType = null, string formatString = null, bool groupSummary = false)
        {
        ...
        private void ucTongHopXetNghiem_May_DoiTuong_Load(object sender, EventArgs e)
        {
