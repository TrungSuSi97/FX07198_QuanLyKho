...
{
    public partial class ucKetQua_TongHop3_NormalGrid : UserControl
    {
        public ucKetQua_TongHop3_NormalGrid()
        {
            InitializeComponent();
        }
        ...
        private void btnXuatExcel_Click(object sender, EventArgs e)
        {
            ...
            string[] headerLeft = new string[] { "SỞ Y TẾ CẦN THƠ", "BVĐK HÒA HẢO - MEDIC CẦN THƠ", "KHOA XÉT NGHIỆM" };
            ...
            if (dtTemp != null && dtTemp.Rows.Count > 0)
            {
                ...
                foreach (DataRow drSID in dtResult.Rows)
                {
                    ...
                    foreach (var item in dmxn)
                    {
                        ...
                        if (chkKetQuaTRongDauNgoac.Checked)
                            sbResult.AppendFormat("{0}{1}:({2})", (countI > 1 && !newGroup ? "; " : ""), maGhep, item.KetQua == null ? "  " : item.KetQua.Trim());
                        else
                            sbResult.AppendFormat("{0}{1}:{2}", (countI > 1 && !newGroup ? "; " : ""), maGhep, item.KetQua == null ? "  " : item.KetQua.Trim());
    ...
    int width = 70, int? minWidth = null, int? maxWidth = null, FormatType? formatType = null, string formatString = null, bool groupSummary = false)
        ...
        private void ucKetQua_TongHop3_NormalGrid_Load(object sender, EventArgs e)
        {
